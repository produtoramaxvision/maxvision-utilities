...
- **SE2 — ✅ PARCIAL SHIPPED (2026-06-21, media-forge v0.2.8): cobertura KLING.** Galeria agora grava a geração tenant-atribuída no path async (webhook) + sync do **kling**. Infra entregue: migration `008-video-jobs-tenant.sql`, `cost-tracker.setJobTenant`/`getJobRecord.tenantId`, helper `recordGalleryFromJob` (minio_key + skip-log), 3 webhook handlers fiados, e2e + 25 testes. Plano CEO+Eng reviewed: `.maxvision/plans/2026-06-21-se2-gallery-tenant-webhook.md`.
  - **SE2-B — FOLLOW-UP (plano próprio):** seedance/bytedance + higgsfield **não entram na galeria ainda**. Motivo provado na impl: o custo deles é gravado no **`pollStatus`** do provider, não no webhook (webhook só ACK), e **não há handler MCP** que chame pollStatus com `galleryStore` em escopo (`media_seedance_poll` não existe; higgsfield `pollStatus` nem grava custo). Cobrir = construir captura de custo no provider + handler de poll → é a obra deferida **pós-EXT1** (acoplada a SE1/F-D). Money-coupled → plano + review antes de codar.
...
- **OPS4 — ✅ FEITO (2026-08-01):** `docker/setup-qemu-action`, `setup-buildx-action` e `login-action` em **v4** (runtime Node 24), `gitleaks-action` em **v3**, `actions/checkout` em **v7**. Cada bump conferido contra as release notes do próprio projeto, não assumido: a única mudança de comportamento do checkout v7 é bloquear fork PR em `pull_request_target`/`workflow_run`, e nenhum workflow aqui usa esses gatilhos; as duas actions `setup-*` são chamadas sem input nenhum, então a remoção de inputs deprecados na v4 não as toca.
- **OPS5 — ⏳ PRECISA DA SUA DECISÃO (verificado 2026-08-01):** `git worktree prune` não remove nada — os dois diretórios (`agent-a439055b5f204c475` 346 MB, `lane-f-g` 138 MB) **não têm `.git`**, deixaram de ser worktrees e viraram árvores de arquivo soltas. Congelados desde 2026-06-02, gitignored. **Não removidos por mim:** sem metadados git não dá para provar que não há trabalho não commitado dentro, e são 484 MB do seu disco. Comando em `TODOS.md`.
...
- **OPS8 — ✅ CONTINUA VÁLIDO, e eu tentei removê-lo por engano (2026-08-01).** Tirei o `continue-on-error` alegando que a causa tinha passado, citando quatro runs em que o passo `Test` do Windows reportava `success`. Essa evidência era produzida pela própria flag: `continue-on-error: true` marca o passo como `success` mesmo em exit≠0. O `main` ficou vermelho na run seguinte, com `Execution of PostgreSQL by a user with administrative permissions is not [permitted]` — o runner roda como admin e o postgres recusa. **A justificativa original abaixo estava certa.** Restaurado, com aviso inline de que só o LOG do passo vale como evidência aqui.
- **(original, confirmado) OPS8 — 2026-06-21, commit `8775418`: Windows CI não-bloqueante.** O runner `windows-latest` não consegue bootstrapar o `embedded-postgres` (`pg_ctl` falha) → suites pg-backed jogam erro na coleta. Não é bug de código e não tem fix runner-side. `ci.yml` mantém Windows pra typecheck/lint/build (sinal real; dev box é Windows) mas marca o step **Test** `continue-on-error` só no Windows. Ubuntu segue como gate autoritativo. Para de bloquear todo merge.
