...
**Known execution-time:** (1) teste de concorrência pode flapar por abort serializável 40001 — a Task 5 Step 4 instrui adicionar retry-on-40001 ao `reserveAtomic` se ocorrer. (2) `entriesForWithTtl` precisa ser adicionado ao store (Task 7 nota). (3) Postgres de teste necessário pros testes `.int.` — sem ele, fazem skip (não quebram o CI puro).
