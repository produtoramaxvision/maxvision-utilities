...
describe('generateVideoWithRefs', () => {
  ...
  it('gemini mode strips personGeneration and generateAudio from payload', async () => {
    mock.queueVideoOperation('op-refs-gemini');
    await generateVideoWithRefs(makeInput(), makeClient(mock));
    const call = mock.recordedCalls[0];
  ...
  it('negativePrompt reaches the request config', async () => {
    mock.queueVideoOperation('op-generatevideowithrefs-neg');
    await generateVideoWithRefs(
  ...
  it('negativePrompt absent means the key is absent, not an empty string', async () => {
    mock.queueVideoOperation('op-generatevideowithrefs-noneg');
    await generateVideoWithRefs(makeInput(), makeClient(mock));
    const args = mock.recordedCalls[0]!.args as Record<string, unknown>;
