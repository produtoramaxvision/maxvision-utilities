---
name: media-forge:mf-examples-zh
description: "Use when the user asks for Chinese media-forge video-prompt examples, Chinese prompt patterns, example rewrites, or safe versions of working Chinese video-generation prompts, for any provider (Veo, Kling, Higgsfield, Seedance)."
triggers:
  - "Chinese prompt example"
  - "中文 提示词"
  - "Chinese example rewrite"
allowedTools: [Read, Grep]
---

# media-forge:mf-examples-zh

Use examples as patterns, not as content to copy blindly. Chinese prompts should stay compact, concrete, and production-oriented. The craft (subject, action, camera, light, sound, constraint) generalizes across providers; the reference tags in the worked examples below (`@Image1`, `@Video1`, `@Audio1`, `@图片1`, `@视频1`) are **Seedance-specific** reference-fusion syntax — preserve them exactly when the example targets Seedance, and see `[skill:mf-video-prompt]` for how they bind. For Kling or Higgsfield, adapt the same craft language to that provider's own reference mechanism (`kling-prompting`, `higgsfield-prompting`).
...
| `provider-specific` | Depends on the active video/API provider. |
...
If the prompt contains protected names, translate the intent into original descriptors: `知名IP角色` becomes `原创蒙面屋顶信使`; `某导演风格` becomes `低饱和胶片质感、硬侧光、长焦压缩空间、静默表演`. See `[skill:mf-safety-rewrite]` for the full method.

## Output Contract

Return the Chinese example, label, risk note, and safer Chinese variant when needed. Keep final video prompt text natural-language unless the user asks for structured output.
