---
name: media-forge:mf-examples-ja
description: "Use when the user asks for Japanese media-forge video-prompt examples, Japanese prompt patterns, example rewrites, or safe versions of working Japanese video-generation prompts, for any provider (Veo, Kling, Higgsfield, Seedance)."
triggers:
  - "Japanese prompt example"
  - "日本語 プロンプト"
  - "Japanese example rewrite"
allowedTools: [Read, Grep]
---

# media-forge:mf-examples-ja

Use Japanese examples as native prompt patterns, not translated English templates. The craft (subject, action, camera, light, sound, constraint) generalizes across providers; the reference tags in the worked examples below (`@Image1`, `@Video1`, `@Audio1`) are **Seedance-specific** reference-fusion syntax — preserve them exactly when the example targets Seedance, and see `[skill:mf-video-prompt]` for how they bind. For Kling or Higgsfield, adapt the same craft language to that provider's own reference mechanism (`kling-prompting`, `higgsfield-prompting`).
...
If the prompt contains protected names, rewrite the creative function into original Japanese descriptors: `有名キャラクターそのもの` becomes `オリジナルの仮面をつけた屋上配達員`; `特定作品そっくり` becomes `低彩度の夜景、硬いサイドライト、静かな演技、長焦点の圧縮感`. See `[skill:mf-safety-rewrite]` for the full method.

## Output Contract

Return the Japanese example, label, risk note, and safer Japanese variant when needed. Keep final video prompt text natural-language unless the user asks for structured output.
