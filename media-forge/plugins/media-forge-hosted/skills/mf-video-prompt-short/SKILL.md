---
name: media-forge:mf-video-prompt-short
description: "Use when the user asks for a compact media-forge video prompt, short Chinese prompt, prompt compression, 30-100 word output, or removal of unnecessary prompt language, for any provider (Veo, Kling, Higgsfield, Seedance)."
triggers:
  - "compact prompt"
...
# media-forge:mf-video-prompt-short

Compress video prompts without losing the production signal. A short prompt still needs mode, subject, action, camera, light, sound when useful, and constraints. Remove filler before removing physical details. Applies across providers; the `@ImageN`/`@VideoN`/`@AudioN` tag preservation rule below is **Seedance-specific** — other providers preserve their own reference identifiers the same way.
...
| I2V | `[Reference] preserved; only [motion/light/camera] changes. Camera: [one move]. Sound: [cue]. Constraint: [what must not change].` |
| V2V | `[Video reference] controls [motion/camera/timing] only; new subject [anchor]. [Action]. Do not transfer [identity/scene/logo].` |
| Chinese | `@Image1为参考，严格保持[主体]不变；仅加入[动作/光线/镜头]。声音：[提示]。` (Seedance-specific `@ImageN` binding) |
