...
Clip 03 chains from Clip 02's accepted footage: `extension_depth: 2` - exactly at `scene_01`'s `max_chain_depth`. That is legal, and it is also the end of the chain: a third output-sourced generation would exceed the cap, and validation should fail it with instructions to open from canonical references instead. **Capability gap:** media-forge does not ship an equivalent `project_state_check.py` validator today. `schemas/project-state.schema.json` (absorbed as `skills/_shared/schemas/project-state.schema.json`) defines the shape, but there is no CLI or test wired up to validate a live `project-state.json` against it. Validate manually against the schema (e.g. with `ajv` or a Zod port) until this lands. This is scheduled re-anchoring: identity decays along output-sourced chains, so the reset is routine, planned at the scene boundary - an intentional cut, opened from `@Image 1`, depth back to 0 - not an emergency repair after drift is visible. If drift appears **before** the cap, that is an immediate `reanchor_after_drift` instead.
