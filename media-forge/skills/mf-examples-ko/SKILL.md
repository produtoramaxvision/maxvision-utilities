---
name: media-forge:mf-examples-ko
description: "Use when the user asks for Korean media-forge video-prompt examples, Korean prompt patterns, example rewrites, or safe versions of working Korean video-generation prompts, for any provider (Veo, Kling, Higgsfield, Seedance)."
triggers:
  - "Korean prompt example"
  - "한국어 프롬프트"
  - "Korean example rewrite"
allowedTools: [Read, Grep]
---

# media-forge:mf-examples-ko

Use Korean examples as native prompt patterns, not translated English templates. The craft (subject, action, camera, light, sound, constraint) generalizes across providers; the reference tags in the worked examples below (`@Image1`, `@Video1`, `@Audio1`) are **Seedance-specific** reference-fusion syntax — preserve them exactly when the example targets Seedance, and see `[skill:mf-video-prompt]` for how they bind. For Kling or Higgsfield, adapt the same craft language to that provider's own reference mechanism (`kling-prompting`, `higgsfield-prompting`).
...
If the prompt contains protected names, rewrite the creative function into original Korean descriptors: `유명 캐릭터 그대로` becomes `오리지널 마스크를 쓴 옥상 배달원`; `특정 작품과 똑같이` becomes `낮은 채도 야경, 강한 사이드 라이트, 조용한 표정 연기, 망원 압축감`. See `[skill:mf-safety-rewrite]` for the full method.

## Output Contract

Return the Korean example, label, risk note, and safer Korean variant when needed. Keep final video prompt text natural-language unless the user asks for structured output.
