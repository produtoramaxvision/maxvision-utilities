---
name: media-forge:mf-characters
description: "Use when the user asks for character consistency, character tags, identity lock, multi-character blocking, wardrobe continuity, hand safety, expression control, or likeness-sensitive character guidance for any media-forge video provider (Veo, Kling, Higgsfield, Seedance)."
triggers:
...
# media-forge:mf-characters

Use this for identity, consistency, multi-character blocking, wardrobe continuity, hand safety, expression control, and likeness-sensitive character guidance. Character prompting must remove ambiguity before adding style. This applies across providers; Higgsfield's Soul ID gives persistent trained identity as a dedicated mechanism (see `higgsfield-prompting`), while Kling Elements and Seedance reference-fusion achieve similar consistency through per-call reference binding — the character-discipline rules below apply regardless of which mechanism carries the identity.
...
Assign each character a stable tag: `Character A`, `Character B`, an image-reference subject, or a user-provided original name. After more than one character appears, do not use ambiguous pronouns. Keep tag, role, appearance, wardrobe, position, action, and emotional beat consistent.

| Field | Prompt use |
|---|---|
| Tag | `Character A` or `<reference> subject` |
...
Field-observed from Chinese production practice; the strongest known stabilizer for multi-person scenes, and it generalizes well beyond its origin provider. Give every visible person an action from exactly one tier:
...
For real-person likeness, do not infer consent from an uploaded asset. Treat portrait, face, and voice workflows as authorization-dependent and provider-specific — Higgsfield Soul ID training, for instance, requires the user's own source images. If authorization is unclear, rewrite to an original character archetype while preserving the scene function. See `[skill:mf-safety-rewrite]` for the full IP/likeness rewrite method.
