...
| Sub-skills | 27 `skills/mf-*/SKILL.md` | When the task is that skill's job. Each is medium-weight: intent, contracts, and routing, not databases. |
...
- `mf-recipes` + `examples-by-mode.md` own the **skeletons** - copy-ready starting patterns.
...
Root `SKILL.md` owns only the Sequence Gate and invariants. `skills/mf-sequence` owns global planning and current-clip compilation. `skills/mf-continuation` owns accepted-footage continuation and re-anchoring. Dense state details live in `references/sequence-project-state.md`, `references/continuation-handoff.md`, and `references/prompt-compiler.md`.
