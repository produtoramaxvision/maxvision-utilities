---
name: media-forge:mf-interview-short
description: "Use when the user wants a fast media-forge creative brief, a short interview, a compressed intake flow, or a quick director-style clarification before prompt writing, on any provider (Veo, Kling, Higgsfield, Seedance)."
triggers:
  - "quick brief"
  - "fast creative brief"
  - "short interview"
...
# media-forge:mf-interview-short

Use this when speed matters more than exhaustive creative discovery. The goal is to turn a vague idea into a compact director brief with no more than three questions, then route to prompt writing for whichever provider the job needs.
...
Route to `[skill:mf-sequence]` for connected clips, long scenes, unclear total duration, or continuation-ready planning; `[skill:mf-continuation]` for accepted-footage continuation; `[skill:mf-video-prompt]` for a full standalone production prompt (or `kling-prompting` / `higgsfield-prompting` when the target provider is already decided); `[skill:mf-video-prompt-short]` for a compact prompt; `[skill:mf-safety-rewrite]` for IP/likeness/filter risk; or `[skill:mf-troubleshoot]` when the user starts from a bad result.
