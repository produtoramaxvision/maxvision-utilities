---
name: media-forge:mf-sequence
description: "Use when a media-forge video request is a long story, connected set of clips, multi-generation scene, campaign sequence, dense storyboard, continuation-ready plan, or any idea that must be divided into stateful clips, on any provider (Veo, Kling, Higgsfield, Seedance)."
triggers:
  - "connected clips"
  - "long story sequence"
  - "campaign sequence"
  - "dense storyboard"
  - "continuation-ready plan"
allowedTools: [Read, Grep]
---

# media-forge:mf-sequence

Use this when the user's idea is larger than one reliable generation, when connected clips are requested, or when the user says continue, extend, next part, part two, next scene, or make it longer. Plan globally, generate locally: the skill plans the whole story, but compiles only the next unresolved clip. This planning discipline is provider-agnostic; the specific duration budgets, reference-tag conventions, and chaining mechanisms vary by provider (Seedance i2v chaining, Kling's `media_kling_video_extend` hops, etc.) and are resolved per clip against the active provider's own profile.
...
Classify as `sequence_project` when the story exceeds the verified active-provider duration, asks for multiple connected clips, contains several narrative beats, is a film scene, ad, campaign, music sequence, action scene, dialogue scene, or uses continue/extend/next-part language. Otherwise classify as `standalone_clip` and return to the concise prompt path.

For every request also classify:

- generation input mode: T2V, I2V, V2V, R2V, FLF2V, edit, native extend when verified for the active provider, or troubleshoot;
...
- provider profile: exact reference-tag convention, verified duration range, prompt budget, supported reference roles, timeline syntax, edit/extension availability, audio behavior, and constraints.

If the provider is unknown, use a conservative generic profile. Do not invent a duration, prompt limit, reference count, or tag syntax.
...
5. Divide each scene into generation-sized clips using the active provider budget or conservative assumption; chain clips from accepted footage only inside a scene, and open every scene from canonical references.
...
Use beginner-friendly language. It is valid to say: "This idea needs three connected generations. I will plan the complete story now, but finalize one prompt at a time so each new prompt matches what the provider actually produced."
...
8. Clip 01 final video prompt in natural language, in the target provider's dialect (see `[skill:mf-video-prompt]`, `kling-prompting`, or `higgsfield-prompting`).
